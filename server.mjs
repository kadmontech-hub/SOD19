...
const headers={
  'X-Content-Type-Options':'nosniff','X-Frame-Options':'DENY','Referrer-Policy':'strict-origin-when-cross-origin','Permissions-Policy':'camera=(), microphone=(), geolocation=(), gyroscope=(self), accelerometer=(self)','Cross-Origin-Opener-Policy':'same-origin','Content-Security-Policy':"default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https://i.imgur.com https://i.ytimg.com https://img.youtube.com; connect-src 'self' https://i.imgur.com; media-src 'self' https:; frame-src 'self' https:; worker-src 'self'; manifest-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'"
};
...
async function handleApi(req,res,url){
  ...
  if(url.pathname==='/api/dialogue'&&req.method==='POST'){if(limited(req,'dialogue',20))return json(res,429,{error:'Pausa un momento antes de continuar'});const input=await body(req);const message=sanitize(input.message,6000);const clientMessageId=sanitize(input.clientMessageId,120);const conversationId=sanitize(input.conversationId,120)||null;if(message.length<2)return json(res,400,{error:'La pregunta está vacía'});return json(res,200,{id:crypto.randomUUID(),clientMessageId:clientMessageId||null,conversationId,reply:dialogueReply(message),mode:'scripted',disclaimer:'SØD ofrece reflexión guiada; no reemplaza asistencia médica, psicológica, legal o profesional.'})}
